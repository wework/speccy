...
const wsGetDefaultState = require('oas-schema-walker').getDefaultState;
...
function validateSchema(schema) {
    ...
    if (errors && errors.length) {
        throw JSONSchemaError('Failed OpenAPI v3 schema validation', { errors });
    }
        ...
        if (o === '$ref') {
            ...
            if (op.callbacks) {
                ...
                for (let c in op.callbacks) {
                    ...
                    if (callback.$ref) {
                        lintObject('reference',callback,options);
                    }
                    else {
...
function validateSync(openapi, options, callback) {
    ...
    if (openapi.components && openapi.components.callbacks) {
        ...
        for (let c in openapi.components.callbacks) {
            ...
            if (cb.$ref) {
                lintObject('reference',cb,options);
            }
            else {
                for (let exp in cb) {
                    let cbPi = cb[exp];
    ...
    if (openapi.components && openapi.components.links) {
        ...
        for (let l in openapi.components.links) {
            ...
            if (link.$ref) {
                lintObject('reference',link,options);
            }
            else {
