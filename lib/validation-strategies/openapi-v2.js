...
const openapi2Schema = require('../../schemas/openapi-2.0.json');
const validateOpenAPI2 = ajv.compile(openapi2Schema);
...
function validateSchema(schema) {
    ...
    if (errors && errors.length) {
        throw JSONSchemaError('Failed OpenAPI v2 schema validation', { errors });
    }
...
function checkSubSchema(schema, parent, state) {
    ...
    lintObject('schema', schema, state.options);
    schema.should.be.an.Object();
    schema.should.not.be.an.Array();

    if (typeof schema.$ref !== 'undefined') {
        schema.$ref.should.be.a.String();
        lintObject('reference',schema,state.options);
    ...
    for (let ct in content) {
        ...
        if (contentType.examples) {
            ...
            for (let e in contentType.examples) {
                ...
                if (ex.$ref) {
                    lintObject('reference',ex,options);
                }
                else {
...
function checkServer(server, options) {
    ...
    if (typeof server.variables !== 'undefined') {
        ...
        for (let v in server.variables) {
            ...
            lintObject('serverVariable',server.variables[v],options);
            options.context.pop();
        }
        should(Object.keys(server.variables).length).be.exactly(srvVars);
        options.context.pop();
    }
    lintObject('server',server,options);
}
...
function checkLink(link, openapi, options) {
    if (link.$ref) {
        ...
        if (options.lint) lintObject('reference',link,'$ref',options);
...
function checkHeader(header, contextServers, openapi, options) {
    if (header.$ref) {
        ...
        lintObject('reference',header,options);
    ...
    lintObject('header',header,options);
}
...
function checkResponse(response, contextServers, openapi, options) {
    if (response.$ref) {
        ...
        lintObject('reference',response,options);
    ...
    lintObject('response',response,options);
}
...
function checkParam(param, index, path, contextServers, openapi, options) {
    ...
    if (param.$ref) {
        ...
        lintObject('reference',param,options);
        var ref = param.$ref;
    ...
    param.in.should.equalOneOf('query', 'header', 'path', 'formData', 'body');
    if (param.in === 'path') {
    ...
    if (param.in === 'body') {
        param.should.have.property('schema');

        checkSchema(param.schema, emptySchema, 'schema', openapi, options);
    }

    if (param.in !== 'body') {
        // param.should.have.property('items');
        // param.should.have.property('collectionFormat');
        param.should.have.property('type');
        if (param.type === 'array') {
            param.should.have.property('items');
            param.items.should.be.type('object');
        }
    }
    ...
    lintObject('parameter',param,options);
    options.context.pop();
    return param;
}
...
function checkSecurity(security,openapi,options) {
    ...
    for (let sr of security) {
        ...
        for (let i in sr) {
            ...
            let sec = jptr.jptr(openapi,'#/securityDefinitions/'+i);
            sec.should.not.be.exactly(false,'Could not dereference securityDefinition '+i);
            if (sec.type !== 'oauth2') {
                ...
                if (!options.allScopes[i]) {
                    ...
                    if (sec.flow == "password") Object.assign(options.allScopes[i],sec.scopes);
                    if (sec.flow == "implicit") Object.assign(options.allScopes[i],sec.scopes);
                    if (sec.flow == "authorizationCode") Object.assign(options.allScopes[i],sec.scopes);
                    if (sec.flow == "clientCredentials") Object.assign(options.allScopes[i],sec.scopes);
                }
    ...
    lintObject('security',security,options);
    options.context.pop();
}
...
function validateSync(openapi, options, callback) {
    ...
    openapi.should.not.have.key('openapi');
    openapi.should.have.key('swagger');
    openapi.swagger.should.have.type('string');
    should.ok(openapi.swagger.startsWith('2.'), 'Must be an OpenAPI 2.x document');
    //openapi.should.have.key('host');
    //openapi.should.have.key('basePath');
    //openapi.should.have.key('schemes');
    openapi.should.have.key('paths');
    should(openapi.paths).be.an.Object();
    //openapi.should.have.key('definitions');
    //openapi.should.have.key('responses');
    //openapi.should.have.key('security');
    //openapi.should.have.key('securityDefinitions');
    //openapi.should.have.key('tags');
    //openapi.should.have.key('externalDocs');
    //openapi.should.have.key('produces');
    //openapi.should.have.key('consumes');

    for (let k in openapi) {
        if (!k.startsWith('x-')) {
            should(['swagger', 'info', 'host', 'basePath', 'schemes', 'consumes','produces','paths', 'definitions','parameters','responses', 'securityDefinitions', 'security', 'tags', 'externalDocs'].indexOf(k)).be.greaterThan(-1,'OpenAPI object cannot have additionalProperty: '+k);
        }
    }
    ...
    if (openapi.info.license) {
        ...
        lintObject('license', openapi.info.license, options);
        options.context.pop();
    }
    ...
    if (typeof openapi.info.contact !== 'undefined') {
        ...
        lintObject('contact',openapi.info.contact,options);
    ...
    lintObject('info',openapi.info,options);
    options.context.pop();
    ...
    validateOpenAPI2(openapi);
    const errors = validateOpenAPI2.errors;
    if (errors && errors.length) {
        throw new JSONSchemaError('Failed OpenAPI v2 schema validation', { errors });
    }

    options.valid = !options.expectFailure;
    lintObject('openapi',openapi,options);
...
function checkPathItem(pathItem, path, openapi, options) {
    ...
    for (let o in pathItem) {
        ...
        if (o === '$ref') {
            ...
            lintObject('reference', pathItem, options);
        }
            ...
            lintObject('operation',op,options);
        }
    ...
    lintObject('pathItem',pathItem,options);
    return true;
}
